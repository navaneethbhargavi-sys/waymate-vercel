...
async function api(path, options = {}, retry = true) {
  if (window.parent !== window) {
    ...
    if (fresh) liveClerkToken = fresh;
    else if (!liveClerkToken) throw new Error("Your session is still loading. Please try again.");
  }
...
const store = {
  get(key, fallback) {
    try {
      const raw = localStorage.getItem("waymate:" + key);
  ...
  set(key, value) {
    ...
    try { localStorage.setItem("waymate:" + key, JSON.stringify(value)); } catch (e) {}
  },
  del(key) {
    delete memoryStore[key];
    try { localStorage.removeItem("waymate:" + key); } catch (e) {}
  }
};

function notificationsEnabled() { return store.get("notifications", true) !== false; }
  ...
  realtimeSource.addEventListener("message", event => {
    try {
      ...
      if (state.chatWith && state.chatWith.id === message.senderId) {
        ...
        notifyUser("New Waymate message", "New message from a buddy");
      }
    } catch (error) {
...
async function sendBuddyRequest(id) {
  ...
  try {
    ...
    notifyUser("Waymate request", "Buddy request sent");
  } catch (e) {
...
function openSettings() {
  ...
  body.innerHTML = '<div class="settings-list">' +
    '<button class="settings-row" onclick="toggleNotifications()"><span>🔔<b>Notifications</b><small>Request and message alerts</small></span><span class="settings-value">' + (notifications ? 'On' : 'Off') + '</span></button>' +
    '<button class="settings-row" onclick="toggleNotifications()"><span>🔕<b>' + (notifications ? 'Turn notifications off' : 'Turn notifications on') + '</b><small>Control realtime message alerts</small></span><span>›</span></button>' +
